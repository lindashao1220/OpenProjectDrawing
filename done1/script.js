class Particle {
  constructor(x, y, size) {
    ...
    this.velocity = createVector(random(-1.6, 1.6), random(-1.6, 1.6));
...
if(g <= 50  && g >= -50){
let wu = Math.abs(g);
xShift = wu * sin(2*PI*phase/period); //amplitude = 200pxs, 4s a round
...
if (y < height/4) {
 ...
 if (averageFrequency > -85) {
  ...
  if (currTime - lastCircleTime > circleInterval) {
    ...
    outerPetal[curCircle].color = color(hong-random(50),lu-random(30),lan-random(30));
